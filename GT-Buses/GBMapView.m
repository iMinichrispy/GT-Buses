...
@interface GBMapView () <RequestHandlerDelegate, CLLocationManagerDelegate> {
...
- (void)handleError:(RequestHandler *)handler code:(NSInteger)code message:(NSString *)message {
    ...
    if ([_delegate respondsToSelector:@selector(handleError:code:message:)]) {
        [_delegate handleError:self code:code message:message];
    }
